...
<h3>The same curve, told as moving mass</h3><p>Here is a second way to build exactly the same Kaplan–Meier curve (Efron, 1967), now with the course's 58 treated patients. Give every patient an equal share of mass, 1/58. Sweep time to the right. At an event, that patient's mass falls out and the curve steps down by exactly that amount. At a censoring nothing falls: the patient's mass is shared equally among everyone still at risk to the right, and their dots grow.</p>
<div class="predict" data-options="It steps down by that patient's mass|It does not move|It steps up a little" data-answer="1" data-hint="Censoring is not an event. The mass stays among the patients still at risk and only changes hands, so the total still on the line, which is Ŝ(t), is unchanged.">Predict: when a patient is censored and their mass is handed to the patients to their right, what happens to Ŝ(t)?</div>
<div data-figure="redistribute"></div>
<p>By the end, each survivor's area is 1/Ĝ(t−), where Ĝ is the Kaplan–Meier curve of <em>remaining uncensored</em> (censoring treated as the "event"). That is where inverse probability of censoring weights come from: KM already gives every survivor 1/Ĝ times their own share. It also shows KM's assumption. The heirs are everyone to the right, so KM assumes whoever left was like everyone who stayed. Tap a dot to read its weight.</p>
